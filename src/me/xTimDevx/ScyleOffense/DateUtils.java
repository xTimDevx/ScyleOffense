	  ...
	  return timePattern.matcher(input).replaceFirst("").trim(); 
	    ...
	    public static String advancedTicksToString(long ticks) { 
	     int hours = (int) Math.floor(ticks / (double) SECONDS_PER_HOUR); 
	        ...
	        if (hours > 0) { 
	            output.append(hours).append(" ").append(hours == 1 ? "§fhour§c" : "§fhours§c"); 
	        } 
	         
	        if (minutes > 0) { 
	            if (output.length() > 0) { 
	                output.append(" §fand§c "); 
	            } 
	             
	            output.append(minutes).append(" ").append(minutes == 1 ? "§fminute§c" : "§fminutes§c"); 
	        } 
	         
	        if (seconds > 0) { 
	            if (output.length() > 0) { 
	                output.append("§f and§c "); 
	            } 
	             
	            output.append(seconds).append(" ").append(seconds == 1 ? "§fsecond§c" : "§fseconds§c"); 
	        } 
